...
void spin_rotors() {
    ...
    //if the notch of the fastest rotor is hit, middle rotor moves. 
    //Case of double stepping, middle rotor also moves when its own notch is hit 
    if(rotor_offsets[0] == turnovers[0] || rotor_offsets[1] == turnovers[1]){
        rotor_offsets[1] = (rotor_offsets[1] + 1) % ALPHABET_SIZE;
    }

    //slowest rotor moves when the notch of middle rotor is hit
